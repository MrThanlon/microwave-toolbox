    ...
    if (Q < 0) {
      ...
      if (Q == 0 && rs == rl) {
        ...
        if (Q < sqrt(max(rs, rl) / min(rs, rl) - 1)) {
          ...
          rv = max(rs, rl) / (Q * Q + 1)//document.circuit.rvpi.value=rv
          w = 2 * PI * f
          ...
          ll = q * rv / w
          l = ls + ll
          document.circuit.lpi.value = l * 1e9//ls-c-ll pi network matching
          q = sqrt(rps / rv - 1) //start source matching
          ls = rps / w / q
          if (qs != 0) {
            lps = rps / qs / w
    ...
    if (Q < 0) {
      ...
      if (Q == 0 && rs == rl) {
        ...
        if (Q < sqrt(max(rs, rl) / min(rs, rl) - 1)) {
          ...
          rv = min(rs, rl) * (Q * Q + 1)//document.circuit.rvt.value=rv
          w = 2 * PI * f
          ...
          ll = rv / w / q
          l = ll * ls / (ll + ls)
          document.circuit.lt.value = l * 1e9//ls-c-ll t network matching
          q = sqrt(rv / rs - 1) //start source matching
          ls = q * rs / w - xs / w
